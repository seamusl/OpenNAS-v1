...
class TFKerasBackend(BaseBackend):
    ...
    def create_layer(self, node):
        ...
        if node.type == 'Conv2D':
            parameters.update({
                ...
                'kernel_initializer' : keras.initializers.glorot_normal(seed=random.randrange(10000000))
            })
    ...
    def fully_train_model(self, model, epochs, augment):
        ...
        # Train model
        model.fit_generator(
            generator=datagen.flow(x_train, y_train, seed=random.randrange(10000000), batch_size=cfg['backend']['batch_size']),
